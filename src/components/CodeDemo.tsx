  ...
  const highlightSyntax = (code: string, language: string) => {
    if (language === 'typescript' || language === 'javascript') {
      return code
        ...
        .replace(/\b(const|let|var|function|return|if|else|for|while|class|interface|export|import|async|await|try|catch|throw)\b/g, '<span class="keyword">$1</span>')
        .replace(/\b(string|number|boolean|object|undefined|null)\b/g, '<span class="type">$1</span>')
          ...
          {codeToDisplay.split('\n').map((_, index) => (
            <span 
              ...
              className={`${styles.lineNumber} ${
                codeExample.highlightLines?.includes(index + 1) ? styles.highlighted : ''
              }`}
            >
              {index + 1}
            </span>
