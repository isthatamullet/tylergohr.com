...
export default function ArchitectureDiagram({ architecture }: ArchitectureDiagramProps) {
  ...
  const getNodeIcon = (type: ArchitectureNode['type']) => {
    switch (type) {
      case 'frontend':
        return (
          <svg viewBox="0 0 24 24" fill="currentColor" className={styles.nodeIcon}>
  ...
  const selectedNodeData = selectedNode ? architecture.find(n => n.id === selectedNode) : null
          ...
          <g className={styles.connections}>
            {architecture.map(node => 
              node.connections.map(connectionId => {
                const targetNode = architecture.find(n => n.id === connectionId)
                ...
                return (
                  <path
                    ...
                    className={`${styles.connectionLine} ${
                      (hoveredNode === node.id || hoveredNode === connectionId) ? styles.highlighted : ''
                    }`}
          ...
          <g className={styles.nodes}>
            {architecture.map(node => (
              ...
              >
                ...
                {/* Node Icon */}
                <g transform="translate(15, 15)" style={{ color: getNodeColor(node.type) }}>
      ...
      {selectedNodeData && (
        <div className={`${styles.nodeDetails} scale-in-on-scroll`}>
          <header className={styles.detailsHeader}>
            <div className={styles.detailsIcon} style={{ color: getNodeColor(selectedNodeData.type) }}>
          ...
          <div className={styles.detailsContent}>
            <p className={styles.detailsDescription}>{selectedNodeData.description}</p>
              ...
              <div className={styles.detailsConnections}>
                ...
                <div className={styles.connectionsList}>
                  {selectedNodeData.connections.map(connectionId => {
                    const connectedNode = architecture.find(n => n.id === connectionId)
                    return connectedNode ? (
                      <button
                        key={connectionId}
                        className={styles.connectionTag}
                        onClick={() => setSelectedNode(connectionId)}
                        style={{ borderColor: getNodeColor(connectedNode.type) }}
                      >
                        {connectedNode.label}
                      </button>
                    ) : null
